...
bashsudo apt install -y curl git build-essential pkg-config libssl-dev bzip2 python3
...
bashcp "/mnt/c/Users/NEW USER/Downloads/solana-release-x86_64-unknown-linux-gnu.tar.bz2" ~/solana.tar.bz2

⚠️ Replace NEW USER with your actual Windows username. Run ls /mnt/c/Users/ to find it.

Step 3 — Extract it:
bashtar -xf ~/solana.tar.bz2
Step 4 — Add to PATH permanently:
bashexport PATH="$HOME/solana-release/bin:$PATH"
bashecho 'export PATH="$HOME/solana-release/bin:$PATH"' >> ~/.bashrc
bashsource ~/.bashrc
...
Step 1 — Clone:
bashgit clone https://github.com/pollinet/pollinet.git
...
bashsolana config set --url localhost
Step 5 — Verify the validator is reachable:
bashsolana cluster-version
...
bashsolana-keygen new --no-bip39-passphrase
...
bashecho "SOLANA_URL=http://localhost:8899" > .env
bashecho "WALLET_PRIVATE_KEY=$PRIVKEY" >> .env
...
bashcd pollinet
Step 2 — Build:
bashcargo build --all-targets --examples
...
bash./scripts/test_pollinet.sh --quick
...
bash./scripts/test_pollinet.sh --m1-only
What happens during this demo:

50 nonce accounts are created on the local chain
50 offline transactions are signed locally — no internet needed
A 5-minute offline period is simulated
All 50 transactions are submitted once connection is restored
...
bashls test_results/
bashcat test_results/*/summary.md
The summary shows:
...
🐦 Twitter: @sol_pollinet
🌐 Website: pollinet.xyz
📦 SDK: github.com/pollinet/pollinet
